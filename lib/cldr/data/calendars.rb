  ...
  module Data
    class Calendars < Base
      ...
      def day_periods(calendar)
        {
          :am => doc.xpath("#{calendar.path}/am").first.content,
          :pm => doc.xpath("#{calendar.path}/pm").first.content
        }
      end
      ...
      def eras(calendar)
        ...
        eras = keys.inject([]) do |result, name|
          ...
          result << extract(path, :key   => lambda { |node| node.attribute('type').value.to_i },
      ...
      def fields(calendar)
        doc.xpath("#{calendar.path}/fields/field").inject({}) do |result, node| 
          ...
          result[key] = node.xpath('displayName').first.content
          result
        end
      end
    end
