...
module Cldr
  module Plural
    class Rules < Array
      class << self
        def extract_cldr(source, target)
          xml = File.read(source)
          rules = Cldr::Plural::Rules.parse_cldr(xml).to_ruby
          File.open(target, 'w+') { |f| f.write(rules) }
          rules
        end
    ...
    class Rule < Array
      ...
      def to_ruby
        @condition ||= 'lambda { |n| ' + reverse.inject(':other') do |result, (key, code)|
          code = Cldr::Plural::Rule.parse(code).to_ruby
          "#{code} ? :#{key} : #{result}"
        end + ' }'
      end
    end
